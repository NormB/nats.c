...
static natsStatus
_unmarshalPersistModeType(nats_JSON *json, jsPersistModeType *persist_mode)
{
    natsStatus  s    = NATS_OK;
    char        *str = NULL;
    ...
    if (strcmp(str, jsPersistModeDefaultStr) == 0)
        *persist_mode = js_PersistDefault;
    else if (strcmp(str, jsPersistModeAsyncStr) == 0)
...
static natsStatus
_marshalPersistModeType(jsPersistModeType persist_mode, natsBuffer *buf)
{
    natsStatus  s = NATS_OK;
    const char  *st = NULL;

    if (persist_mode == js_PersistDefault)
        return s;
...
js_marshalStreamConfig(natsBuffer **new_buf, jsStreamConfig *cfg)
{
    ...
    IFOK(s, _marshalPersistModeType(cfg->PersistMode, buf));
