...
_setHeadersFromOptions(natsMsg *msg, jsPubOptions *opts)
{
    ...
    if (s == NATS_OK)
    {
        ...
        else if (opts->ExpectLastSubjectSeq > 0)
        {
            ...
            if ((s == NATS_OK) && (!nats_IsStringEmpty(opts->ExpectLastSubjectSeqSubject)))
            {
                s = natsMsgHeader_Set(msg, jsExpectedLastSubjSeqSubjHdr, opts->ExpectLastSubjectSeqSubject);
            }
        }
    }
    ...
    if (s == NATS_OK && json != NULL)
    {
        ...
        else if (new_puback != NULL)
        {
            ...
            else
            {
                ...
                IFOK(s, nats_JSONGetStr(json, "val", &(pa->Value)));
